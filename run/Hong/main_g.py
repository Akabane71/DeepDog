...
def robot_cup_2024_g_run():
    ...
    # 右走
    if lr == "right":
        h_run.change('./tmp/step_2_right_g.pkl')
        h_run.main()
    else:
        # 左走
        h_run.change('./tmp/step_2_left_g.pkl')
        h_run.main()

    # 前往识别区域
    h_run.change('./tmp/step_3_vision_g.pkl')
    h_run.main()
    ...
    time.sleep(1)
    # 向左移动一段距离
    h_run.change('./tmp/step_3_move_left_g.pkl')
    h_run.main()
    ...
    # 右转前往小球
    h_run.change('./tmp/step_4_go_to_ball_g.pkl')
    h_run.main()
